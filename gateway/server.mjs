...
function createServer(){
  ...
  server.registerTool('maintenance_status',{title:'Maintenance status',description:'Show backup/audit retention settings and current local state size.',inputSchema:z.object({})},async()=>{ const cfg=loadConfig(); return toolText({retention:cfg.maintenance,storage:await stateSummary({backupRoot:BACKUP_ROOT,auditLog:AUDIT_LOG})}); });
  ...
  server.registerTool('active_editor_context',{title:'Active editor context',description:'Return the latest VS Code editor and selection snapshot.',inputSchema:z.object({})},async()=>{ const cfg=loadConfig(); const ctx=vscodeContext(cfg); return toolText({capturedAt:ctx.capturedAt,activeEditor:ctx.activeEditor || null}); });
  server.registerTool('list_diagnostics',{title:'List VS Code diagnostics',description:'Return latest VS Code Problems diagnostics, optionally filtered by severity or path.',inputSchema:z.object({severity:z.enum(['error','warning','information','hint']).optional(),path:z.string().optional(),limit:z.number().int().min(1).max(300).optional()})},async({severity,path:pp,limit=100})=>{ const cfg=loadConfig(); const ctx=vscodeContext(cfg); let items=ctx.diagnostics || []; if(severity) items=items.filter(d=>d.severity===severity); if(pp) items=items.filter(d=>d.path===pp || d.path.endsWith(pp)); return toolText({capturedAt:ctx.capturedAt,diagnostics:items.slice(0,limit),total:items.length}); });
  ...
  server.registerTool('git_raw',{title:'Git raw',description:'Run reviewed Git builtin arguments in the active workspace, e.g. ["status", "--short"].',inputSchema:z.object({workspaceId:z.string().optional(),args:z.array(z.string()).min(1),maxOutputChars:z.number().int().min(1000).max(500000).optional(),timeoutMs:z.number().int().min(1000).max(1800000).optional()})},async({workspaceId,args,maxOutputChars,timeoutMs})=>{ const cfg=loadConfig(); const w=getWs(cfg,workspaceId); if(w.reference) throw new Error('Cannot run git_raw in reference workspace'); assertGitRawWorkspaceBound(args); assertGitAllowed(cfg,args,'Git raw'); const limits=commandLimits(cfg,timeoutMs,maxOutputChars); const r=await runGit(w,args,limits.maxOutputChars,limits.timeoutMs); await audit('git_raw',{workspace:w.id,args,exitCode:r.exitCode}); return toolText({workspace:wsPublic(w),...r}); });
