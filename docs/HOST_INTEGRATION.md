...
Desktop hosts also use one fixed machine-wide Gateway port from the shared config. Fresh desktop state defaults to `8787`. VS Code and Obsidian never scan upward to `8788`, `8789`, or another fallback when that port is busy. A matching DevMate instance is attached; a foreign listener fails closed with an explicit port-conflict diagnostic. Passive host activation and context refresh never claim or rewrite the shared runtime version; a fresh passive desktop config uses an unclaimed runtime version until Start owns the shared startup lease. A newer host promotes the runtime version only after it owns that lease. A running Gateway then self-terminates when the authoritative instance identity, runtime version, or configured port changes; stale-process recovery may additionally retire only a same-instance process proven by a fresh durable ownership lock plus matching loopback health identity. Older hosts may attach to the current Gateway but cannot downgrade the shared version or spawn an older Gateway. This keeps one provider upstream stable across extension/plugin upgrades, reinstalls, and simultaneous VS Code/Obsidian entry points.

Normal activation, update, reinstall, context sync, and configuration writes never change an already-established shared Gateway port. A historical port value can be changed only by the explicit **Repair Gateway Port to Configured Default** lifecycle. Repair first completes shared Stop, acquires the same startup lease used by Start, refuses while a same-instance Gateway PID or running lifecycle remains, verifies the requested target port is free, performs one guarded shared-config port mutation, and then returns through the normal Start/Ready path without changing Current Project. A failed precondition leaves the configured port unchanged.
