...
Browser Control is owner-only. It keeps managed Chromium sessions alive across MCP requests, caps session/tab counts, binds every session to one workspace, and rejects stale semantic element refs after page-changing actions. Remote URLs remain blocked by default; loopback-only mode blocks service workers and fences WebSocket destinations and therefore requires Playwright 1.48 or newer. Snapshots can combine bounded text, ARIA state, semantic element refs/geometry, diagnostics, and an optional inline screenshot. Actions include click/double-click, typing, keyboard input, scrolling, selection, drag/drop, workspace-safe upload/download capture, tab control, waits, and explicit screenshot artifacts.
