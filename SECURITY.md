...
- Workspace scopes are checked for tools, processes, previews, leases, approvals, and work sessions.
...
- Workspace leases, complex work sessions, and approval requests are persisted under the config directory using atomic file replacement.
...
- Mutations, commands, Git operations, team member changes, leases, work sessions, approvals, preview publication, and tool calls produce audit metadata.
...
- `/control/metrics` exposes bounded operational metrics to loopback collectors only. Do not proxy it publicly.
