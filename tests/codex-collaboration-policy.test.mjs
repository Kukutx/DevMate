...
test('shared ON/OFF preference is durable and does not require starting Codex', () => {
  assert.equal(collaboration.codexCollaborationStatus().enabled, false);
  assert.deepEqual(collaboration.configureCodexCollaboration(true), { enabled: true });
  ...
  assert.deepEqual(collaboration.configureCodexCollaboration(false), { enabled: false });
  assert.equal(collaboration.codexCollaborationStatus().enabled, false);
});

test('only one active Codex task is reserved and stale active state is interrupted on restart', () => {
...
test('production runtime loads collaboration only after singleton lock ownership and no magic APPLY command exists', async () => {
  ...
  assert.match(collaborationSource, /assertAgentProposalConflictFree/);
  assert.match(collaborationSource, /safeFileMutationHandler\('write_file'\)/);
});
