...
test('desktop documentation describes one complete provider-neutral OAuth MCP 2026 runtime lifecycle', () => {
  ...
  assert.match(obsidian, /Gateway started or attached[\s\S]*public connection started or attached[\s\S]*OAuth-authenticated MCP server\/discover verified[\s\S]*tools\/list verified[\s\S]*gateway_status tools\/call verified[\s\S]*Ready/i);
  ...
  assert.match(obsidian, /Desktop public MCP defaults to OAuth/i);
  ...
  assert.match(host, /Start is Gateway → public connection → OAuth MCP 2026 preflight → Ready/i);
  ...
  assert.match(host, /Public Ready requires OAuth; no-auth is loopback-only/i);
  ...
  assert.match(tunnels, /Remote `\/mcp` requests require OAuth/i);
