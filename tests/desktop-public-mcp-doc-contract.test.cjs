...
test('desktop documentation describes one complete provider-neutral MCP 2026 lifecycle with default no-auth and optional OAuth', () => {
  ...
  assert.match(obsidian, /Gateway started or attached[\s\S]*public connection started or attached[\s\S]*MCP server\/discover verified with the selected authentication mode[\s\S]*tools\/list verified[\s\S]*gateway_status tools\/call verified[\s\S]*Ready/i);
  ...
  assert.match(obsidian, /Public MCP defaults to no authentication; OAuth is optional/i);
  ...
  assert.match(host, /Start is Gateway → public connection → MCP 2026 preflight using the selected authentication mode → Ready/i);
  ...
  assert.match(host, /Public Ready supports default no-auth and optional OAuth/i);
  assert.match(host, /Desktop public MCP defaults to no authentication/i);
  ...
  assert.match(tunnels, /auth\.mode: "none".*default.*ngrok.*Cloudflare.*external HTTPS ingress/is);
  assert.match(tunnels, /OAuth is optional/i);
