...
test('Gateway purges legacy backup layouts during startup and still trims large audit state', { timeout: 30000 }, async () => {
  ...
  console.error('[large-state] fixture preparation started');
  try {
    workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'devmate-large-state-workspace-'));
    ...
    console.error(`[large-state] fixture preparation completed in ${Math.round(performance.now() - fixtureStartedAt)}ms`);
    ...
    console.error('[large-state] Gateway startup started');
    const startedAt = Date.now();
    const result = await controller.start({ timeoutMs: 10000 });
    const readyMs = Date.now() - startedAt;
    console.error(`[large-state] Gateway startup completed in ${readyMs}ms`);
    ...
    console.error('[large-state] maintenance wait started');
    ...
    console.error(`[large-state] maintenance wait completed in ${Math.round(performance.now() - maintenanceStartedAt)}ms`);
    ...
    console.error('[large-state] cleanup started');
    try {
      if (controller) await controller.dispose({ stopOwned: true });
    } finally {
      try {
        fs.rmSync(stateDirectory, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
      } finally {
        if (workspaceRoot) fs.rmSync(workspaceRoot, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 });
        console.error(`[large-state] cleanup finished after ${Math.round(performance.now() - cleanupStartedAt)}ms`);
      }
    }
  }
});
...
test('Gateway uses the append-only backup index fast path for thousands of current-format snapshots', { timeout: INDEXED_FIXTURE_TEST_TIMEOUT_MS }, async () => {
  ...
  console.error('[large-state] fixture preparation started');
  try {
    workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'devmate-indexed-workspace-'));
    const configFile = path.join(stateDirectory, 'config.json');
    const stateRoot = path.join(stateDirectory, 'state');
    const backupRoot = path.join(stateRoot, 'backups');
    const port = await freePort();
