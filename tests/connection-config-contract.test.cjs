...
test('Connection Setup mutates only connection capability through the shared instance transaction', () => {
  ...
  assert.match(block, /const stopState = await prepareConnectionMutation\(\)/);
